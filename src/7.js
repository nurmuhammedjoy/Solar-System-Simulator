...
  uranusRing: '/textures/8k_saturn_ring_alpha.png',
  saturnRing: '/textures/8k_saturn_ring_alpha.png'
};
...
function createUranus() {
  ...
  // Uranus sphere
  const geometry = new THREE.IcosahedronGeometry(10, 16);
  const uranusMat = new THREE.MeshStandardMaterial({ map: loader.load(textures.uranus) });
  uranus = new THREE.Mesh(geometry, uranusMat);
  uranus.name = 'uranus';
  uranus.position.set(0, 0, 0);
  uranus.add(createLabel('Uranus', new THREE.Vector3(0, 10, 0)));

  // Add Uranus to scene immediately
  raycastable.push(uranus);
  scene.add(uranus);

  // Load texture and add ring when ready
  loader.load(
    '/texture/uranus_ring_realistic.png',
    function (texture) {
      const ringGeometry = new THREE.RingGeometry(12, 30, 64);
    ...
    undefined,
    function (err) {
      console.error('Failed to load Uranus ring texture:', err);
    }
  );
}
